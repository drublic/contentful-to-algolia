...
class Sync {
  ...
  syncSingle (type, algoliaInstance) {
    return this.contentful.getEntries(type, this.entryId, this.manipulateSingle, this.manipulateEntries)
      .then((content) => {
   ...
   * @param  {Function} manipulateSingle Manipulate each localized entry
   * @param  {Function} manipulateEntries Manipulate all entries
   * @return {void}
   */
  sync (contentTypes, indexName, callback, entryId = false, manipulateSingle = false, manipulateEntries = false) {
    ...
    this.manipulateEntries = manipulateEntries;
